...
export async function GET(request: NextRequest) {
  try {
    ...
    ordersSnapshot.forEach((doc) => {
      ...
      const orderDate = order.deliveredAt?.toDate() || order.timestamp?.toDate();
    ...
    paymentsSnapshot.forEach((doc) => {
      ...
      const paymentStart = payment.period?.start?.toDate();
      const paymentEnd = payment.period?.end?.toDate();
      
      if (paymentStart && paymentEnd &&
          paymentStart.getTime() === startDate.getTime() &&
          paymentEnd.getTime() === endDate.getTime() &&
          payment.status === 'paid') {
